<script lang="ts">
    import { JobApplication, loadApplications }  from "$lib/stores/student/student_store";
                        ...
                        <button class="group bg-[#D2AC72] border border-solid border-[#AB7C7C] w-24 font-inter text-xs font-black text-white h-8 ml-auto mr-0 transition-colors duration-300 relative">
                            ...
                            {:else}
                                ...
                                <form method="POST" action="?/cancel_app">
                                    <button class="hidden group-hover:inline hover:bg-black absolute inset-0 w-full h-full text-white text-xs font-black" formaction="?/cancel_app" name="job_id" value="{application.job_id}">CANCEL</button>
                                </form>
                            {/if}
                        </button>
                    </div>
