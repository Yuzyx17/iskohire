...
<div class="flex">
    <div id="content" class="m-8">
        {#await mockdata}
            <p>Loading...</p>
        {:then mock}
            {#each mock as item}
                <div class="flex flex-col bg-white m-10 p-10 shadow-[0_4px_4px_0px_rgba(0,0,0,0.25)] w-96 font-inter">
                    <h1 class="text-2xl font-semibold">Job Position</h1>
                    <span class="text-xs font-semibold h-24">
                        Company Name
                    </span>
                    <button class="bg-[#D2AC72] border border-solid border-[#AB7C7C] w-24 font-inter text-xs font-black text-white h-8">
                        APPLY
                    </button>
                </div>
            {/each}
