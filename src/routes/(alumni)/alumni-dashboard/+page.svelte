<script lang="ts">
  ...
  import {
    loadApplications,
    Applications,
  } from "$lib/stores/application_store";
      ...
      {#if isPostsLoading}
        {#each Array(5) as _, i}
          <div
            ...
            class="bg-white card card-hover shadow-offset-x-0 shadow-offset-y-4 shadow-blur-4 shadow-spread-0 shadow-opacity-25 w-[277px] min-w-[277px] h-[309px] min-h-[309px] p-3 flex flex-col mr-3"
          />
        {/each}
      {:else if $JobPosts}
        {#each $JobPosts as post}
          <button
            on:click|preventDefault={() => getApplicants(post.job_id)}
            class="bg-white card card-hover shadow-offset-x-0 shadow-offset-y-4 shadow-blur-4 shadow-spread-0 shadow-opacity-25 w-[277px] min-w-[277px] h-[309px] min-h-[309px] p-3 flex flex-col mr-3"
          >
            <div class="flex flex-col">
              <span
                class="text-black font-inter text-xl font-extrabold leading-normal text-center"
              >
                {post.job_title}
              </span>
              <span
                class="text-gray-600 font-inter text-sm font-normal leading-normal text-center"
              >
                {post.desc}
              </span>
            </div>
            <div class="w-full mt-auto self-end">
              <div class="grid grid-cols-2 gap-2 mt-auto mb-0">
                <div class="flex justify-center">
                  <img
                    src="./images/loc_type-employer.png"
                    class="h-3.5 w-3.5 mt-auto mb-auto mr-2"
                    alt="Location Type Logo"
                  />
                  <span
                    class="text-gray-600 font-inter text-xs font-normal leading-normal mt-auto mb-auto"
                    >{post.loc_type}</span
                  >
                </div>
                <div class="flex">
                  <img
                    src="./images/emp_type-employer.png"
                    class="h-3.5 w-3.5 mt-auto mb-auto mr-2"
                    alt="Employment Type Logo"
                  />
                  <span
                    class="text-gray-600 font-inter text-xs font-normal leading-normal mt-auto mb-auto"
                    >{post.employment_type}</span
                  >
                </div>
                <div class="flex justify-center">
                  <img
